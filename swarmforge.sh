...
parse_config() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    local prompt_role
    prompt_role="$(prompt_role_for_role "$role")"
    if [[ "$agent" != "none" && ! -f "$ROLES_DIR/$prompt_role.prompt" ]]; then
      echo -e "${RED}Error:${RESET} Missing role prompt $ROLES_DIR/$prompt_role.prompt"
      exit 1
    fi
...
Read swarmforge/${prompt_role}.prompt, then read every file it refers to recursively, and follow all of those instructions.
Your configured SwarmForge role is: ${role}.
...
EOF
  ...
  if [[ "$prompt_role" == "coder" ]]; then
    cat >> "$prompt_file" <<EOF
When your current task is complete and your role is ready for new pooled work, run: $SWARM_TOOLS_DIR/notify-agent.sh --free ${role}
EOF
  fi
}
